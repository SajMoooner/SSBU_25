...
class Experiment:
    ...
    def __initialize_csv_file(self):
        """Inicializácia CSV súboru so záhlavím."""
        with open(self.accuracies_file, 'w') as file:
            file.write(
                "Model,Replication,Accuracy,F1 Score,ROC AUC,Precision,Best Parameters\n")  # ✅ Pridaný stĺpec Precision

    def run(self, X, y):
        """Spustí experiment cez viaceré replikácie."""
    ...
    def __run_single_replication(self, replication, X, y):
        """Spustí jednu replikáciu trénovania a vyhodnocovania modelov."""
        if self.logger:
    ...
    def __balance_dataset(self, X, y):
        """Vyváženie datasetu pomocou SMOTE."""
        smote = SMOTE()
        return smote.fit_resample(X, y)

    def __train_and_evaluate_model(self, model_name, X_resampled, y_resampled, replication):
        """Trénuje a vyhodnocuje jeden model."""
        # Vytvoríme kópiu modelu, aby každý beh mal svoju vlastnú inštanciu
        ...
        # Použijeme kópiu modelu pre trénovanie s najlepšími parametrami
    ...
    def __store_results(self, model_name, replication, accuracy, f1, roc_auc, precision, best_params):
        """Uloží výsledky jednej evaluácie."""
        new_row = pd.DataFrame({
            ...
            'f1_score': [f1],
            'roc_auc': [roc_auc],
            'precision': [precision],  # ✅ Uloženie precision
            'best_params': [best_params]
        })
        self.results = pd.concat([self.results, new_row], ignore_index=True)

        # Pridanie výsledkov do CSV súboru
    ...
    def __calculate_mean_conf_matrices(self):
        """Vypočíta priemerné matice zámien pre každý model."""
